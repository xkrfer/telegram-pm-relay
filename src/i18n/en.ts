...
• /reset-verification <user_id> - Reset verification limits`,
    ...
    noMapping: '❌ Cannot find original sender (data may have been cleaned or not a forwarded message).',
    sendFailed: '❌ Send failed. The user may have blocked the bot or deleted their account.',

    // Stats
    statsFailed: '❌ Failed to get statistics',

    // Search
    searchNoKeyword: '⚠️ Please provide a search keyword\nUsage: /search <keyword>',
    searchFailed: '❌ Search failed',

    // Template
    templateFormatError: '⚠️ Invalid format\nUsage: /template add <keyword> <content>',
...
3 = Very Strict (1/min, 10/hour)`,
    rateLimitLevelError: '⚠️ Rate limit level must be between 0-3\n0=Normal, 1=Relaxed, 2=Strict, 3=Very Strict',
    ...
    verificationMethodSet: (method) => `✅ Verification method set to: ${method}\n\nUse /verification status to view details`,
...
• ai - AI verification`,
    ...
    verifyUserAlreadyVerified: (userId) => `✅ User ${userId} is already verified`,
    verifyCannotGenerate: (userId) => `❌ Cannot generate verification link for user ${userId}\n\n`,
    verifyCooldown: (duration) => `⏳ Verification cooldown, please wait ${duration}`,
    ...
    verifySendFailed: '❌ Failed to send verification link, please check user ID',
    reverifyCleared: (userId) => `✅ Cleared verification status for user ${userId}\nUser will need to re-verify on next message`,
    reverifyFailed: '❌ Operation failed, please check user ID',
    resetVerificationDone: (userId) => `✅ Reset verification limits for user ${userId}\nUser can now request a new verification link`,
    ...
    userInfoAttempts: (attempts, remaining) => {
      const remainingText = remaining !== undefined ? ` (${remaining}/3 remaining)` : '';
    ...
    recallFailed: '❌ Recall failed (may exceed 48 hours or message already deleted)',
...
If the link has expired, please try again later.`,
    ...
    verificationCooldown: (duration) => `⏳ Verification cooldown, please try again in ${duration}`,
    verificationLimitReached: '⚠️ You have reached the verification limit (3 times/hour)\nPlease try again later',
    verificationStartFailed: '❌ Failed to start verification, please try again later',
    rateLimitedNotify: (name, id, reason) => `⚠️ User triggered rate limit\nUser: ${name} (ID: ${id})\nReason: ${reason}`,
  ...
  verification: {
    // Math
    mathQuestion: (question) => `🧮 Please answer the following question:\n\n${question}`,

    // Quiz
    quizQuestion: (question) => `❓ Please answer the following question:\n\n${question}`,
    quizBank: [
      {
    ...
    // Turnstile
    turnstileMessage: (minutes) => `🔒 First-time users need to complete verification\n\n⏱️ Link expires in ${minutes} minutes`,
    turnstileButton: '🔐 Click to verify',
    turnstileChallenge: 'Please complete Cloudflare Turnstile verification',

    // AI
    aiQuestion: (question) => `🤖 Please answer the following question:\n\n${question}`,
...
If you have questions, please contact the admin.`,
    ...
    cooldownNotReached: (seconds) => `Cooldown not reached (wait ${seconds} seconds)`,
    perMinuteExceeded: (limit) => `Exceeded per-minute limit (${limit} messages)`,
  ...
  verificationService: {
    ...
    cooldownMessage: (duration) => `Verification cooldown, please wait ${duration}`,
  ...
  html: {
    ...
    pageDescription: 'First-time users need to complete human verification before using the bot.',
